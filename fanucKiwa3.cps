/**
  ...
  $Revision: 43164 8ab80bf16c931c603001f9b034b51d038d0c50d0 $
  $Date: 2021-02-04 09:58:04 $
  
  FORKID {04622D27-72F0-45d4-85FB-DB346FD1AE22}
*/

description = "FANUC Kiwa";
vendor = "Fanuc";
...
function onSection() {
  ...
  if (insertToolCall || !lengthCompensationActive || operationNeedsSafeStart || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    var diameterOffset = tool.diameterOffset;
    if (lengthOffset > 99) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(
        ...
        zOutput.format(initialPosition.z), hFormat.format(lengthOffset), dFormat.format(diameterOffset)
      );
      lengthCompensationActive = true;
    }
    zIsOutput = true;
...
function onSectionEnd() {
  if (isInspectionOperation() && !isLastSection()) {
    ...
    writeBlock(properties.stopOnInspectionEnd == true ? onCommand(COMMAND_STOP) : "");
  }
  writeBlock(gPlaneModal.format(17));
